...
using namespace std;
bool Pcomp(const P a, const P b) { return a.X != b.X ? a.X < b.X : a.Y < b.Y; }

// 凸包
VP convexHull(VP ps) { // 元の点集合がソートされていいならVP&に
  int n = ps.size(), k = 0;
  sort(ps.begin(), ps.end(), Pcomp);
  VP ch(2 * n);
...
pair<int, int> convexDiameter(const VP &ps) {
  int n = ps.size();
  int i = min_element(ps.begin(), ps.end(), Pcomp) - ps.begin();
  int j = max_element(ps.begin(), ps.end(), Pcomp) - ps.begin();
  int maxI, maxJ;
  D maxD = 0;
  REP(_, 2 * n) {
...
Graph segmentArrangement(const vector<L> &segs, VP &ps) {
  ...
  REP(i, n) {
    ...
    sort(list.begin(), list.end(), Pcomp);
    REP(j, list.size() - 1) {
      int a = list[j].second;
